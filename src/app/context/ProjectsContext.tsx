import { ApiErrors } from 'components/common/api'
import { createContext, ReactNode, useContext } from 'react'
import { once } from 'lodash'
import { UsePaginatedQueryResult } from 'app/hooks'
import { ApiPagination } from 'components/common/api/ApiPagination'
import { SpinnerOverlay } from 'components/common/layout'

type ApiQueryResult<T> = Partial<UsePaginatedQueryResult<T>>

export type ProjectsContextValue<T = unknown> =
  | ApiQueryResult<T> & {
      children: ReactNode | ((data: T | T[] | undefined) => ReactNode) | null
    }

const createProjectsContext = once(<T,>() =>
  createContext<ProjectsContextValue<T>>({
    children: null,
  }),
)
export const useProjectsContext = <T,>() => useContext(createProjectsContext<T>())

export function ProjectsProvider<T>(queryData: ProjectsContextValue<T>) {
  const ProjectsContext = createProjectsContext<T>()

  return (
    <ProjectsContext.Provider value={queryData}>
      {queryData?.status !== 'success' && queryData?.isFetching ? (
        <SpinnerOverlay />
      ) : (
        <ApiErrors error={queryData?.error} />
      )}

      {queryData?.status === 'success' && (
        <>
          {typeof queryData.children === 'function'
            ? queryData.children(queryData?.items)
            : queryData.children}
        </>
      )}

      <ApiPagination />
    </ProjectsContext.Provider>
  )
}
